...
| Panier et checkout | États vide et rempli présents, Stripe en CHF avec carte et TWINT préparés | Orange | Ajouter une clé Stripe active, tester le paiement sandbox et documenter les emails client |
...
Les coordonnées commerciales confirmées sont désormais : **Bahloul Yacine**, chemin des Lieugex 17, 1860 Aigle, Suisse, `yacbhl71@gmail.com`, téléphone `+41 76 220 41 20`, avec la devise CHF. TWINT est techniquement préparé dans Stripe Checkout, mais sa disponibilité dépend de l’activation du moyen de paiement et de la validation du compte Stripe en Suisse. L’adresse email pourra être remplacée ultérieurement si nécessaire. Les textes de livraison, retours, confidentialité et conditions générales doivent encore être relus avec les informations juridiques et logistiques définitives.
...
Le checkout invite désormais explicitement un visiteur non connecté à se connecter au lieu de le renvoyer silencieusement vers l’accueil, et affiche un message récupérable si Stripe échoue. Un endpoint applicatif signé est disponible sur `/api/stripe/webhook` ; il vérifie la signature Stripe et marque les sessions Checkout payées pour le fulfillment, notamment après un paiement TWINT. Le diagnostic de production a montré que le fallback statique capturait les requêtes POST inconnues et renvoyait l’index HTML avec un statut 200 ; ce fallback est désormais limité aux requêtes GET, afin que le webhook réponde en JSON comme attendu. Le bouton de test du panneau Manus peut toutefois rester « En construction » tant que le sandbox associé n’est pas actif. Lorsque la clé Stripe est absente ou expirée, le serveur signale maintenant une indisponibilité temporaire au lieu de présenter l’échec comme une erreur utilisateur générique. Les zones encore incomplètes sont principalement opérationnelles : les ventes et commandes restent affichées comme indisponibles lorsqu’aucune source réelle n’est branchée, l’activité récente n’est pas encore alimentée par un journal général, et le traitement fournisseur reste manuel. Ces états sont honnêtes et préférables à des chiffres simulés, mais ils doivent être finalisés avant une exploitation à volume réel.

## Audit technique et dépôt

Le serveur de développement a été redémarré et répond correctement. Le typage TypeScript, les 21 fichiers de test et les 66 tests passent. Le build Vite et le bundle serveur réussissent. Le build produit désormais plusieurs chunks dédiés aux familles React, UI, données et graphiques. Le fallback de production a également été restreint aux requêtes GET pour ne pas intercepter les endpoints POST, notamment le webhook Stripe. Le chunk applicatif principal reste d’environ 828 Ko avant compression. Une mesure légère du DOMContentLoaded et du LCP est désormais envoyée au canal analytique Umami lorsque celui-ci est disponible ; une séparation plus fine par routes reste recommandée.
...
Les coordonnées commerciales et la devise CHF sont confirmées et intégrées. TWINT est ajouté à la liste des moyens de paiement Checkout, sous réserve de son activation dans le compte Stripe. Il reste à confirmer l’identité légale complète ainsi que les textes de livraison et de retour. Le paiement Stripe sandbox doit encore être effectué depuis une session MAZIGHO connectée par le propriétaire ; la prise de contrôle n’a pas été possible sur tablette. L’endpoint `/api/stripe/webhook` est prêt, mais son test réel dépend d’un sandbox Stripe actif et d’un secret de signature webhook configuré. Après ce test, vérifier que la commande passe bien dans la file « À commander » et qu’un administrateur peut retrouver les informations nécessaires à l’exécution AliExpress.
